# allml.py   FIXED VERSION
# AllML - All-in-One Machine Learning Library
# Version: 1.0.1  (parameter-mapping hotfix)
...
# 
# Models that need a *float* learning rate
# 
_FLOAT_LR = {
...
# Models whose `learning_rate` param expects a *string schedule*
_SGD_MODELS = {"sgd_regressor", "sgd_classifier"}
_MLP_MODELS = {"mlp_regressor", "mlp_classifier"}

# GradientBoosting alpha must be in (0, 1)  skip user's alpha
_SKIP_ALPHA = {"gradient_boosting_regressor", "gradient_boosting_classifier"}

# SVM models that accept a kernel string
_SVM_MODELS = {"svr", "svc", "linear_svr", "linear_svc", "nu_svr", "nu_svc"}
...
class AllML:
    ...
    def __init__(self, csv_filepath: str):
        ...
        cprint(f" Dataset loaded!", Colors.GREEN, bold=True)
        cprint(f"   Rows: {self._df.shape[0]} | Columns: {self._df.shape[1]}",
               Colors.YELLOW)
    ...
    def method(self, method_name: Union[str, List[str]]) -> "AllML":
        ...
        if method_name == "all":
            if self._task_type is None:
                cprint("️  Call col_to_pred() before method().", Colors.YELLOW)
                return self
            ...
            cprint(f" All {len(self._method_names)} methods for: {self._task_type}",
                   Colors.GREEN)
            return self
        ...
        for m in method_name:
            if m not in all_methods:
                ...
                cprint(f"   Regression    : {list(self.REGRESSION_METHODS.keys())}",
                       Colors.YELLOW)
                cprint(f"   Classification: {list(self.CLASSIFICATION_METHODS.keys())}",
                       Colors.YELLOW)
                cprint(f"   Clustering    : {list(self.CLUSTERING_METHODS.keys())}",
                       Colors.YELLOW)
                raise ValueError(f"Unknown method: {m}")
            validated.append(m)

        self._method_names = validated
        cprint(f" Method(s): {validated}", Colors.GREEN)
        return self
    ...
    def split(self, train_pct: float, test_pct: float,
        ...
        if not self._feed_cols or not self._pred_cols:
            raise ValueError(" Call col_to_feed() and col_to_pred() first.")

        cprint(f"\n Splitting  Train:{train_pct}% Test:{test_pct}% Val:{val_pct}%",
        ...
        cprint(f"   Train: {len(X_train)} | Test: {len(X_test)}"
               + (f" | Val: {len(self._X_val)}" if val_pct > 0 else ""),
               Colors.GREEN)
        print()
        return self
    ...
    def train(self,
        ...
        cprint(f"\n Training  |  task: {self._task_type}", Colors.MAGENTA, bold=True)
        ...
        for mname in self._method_names:
            try:
                cprint(f"\n️  Training: {mname.upper().replace('_',' ')}",
                ...
                #  IsotonicRegression: 1-D only 
                if mname == "isotonic_regression":
                    if self._X_train.shape[1] != 1:
                        cprint(
                            f"   ️  Skipping 'isotonic_regression': needs exactly "
                            f"1 feature, you have {self._X_train.shape[1]}.\n"
                            f"       Use col_to_feed() with a single column "
                            f"for this algorithm.",
                            Colors.YELLOW)
                        continue
                    model.fit(self._X_train[:, 0], self._y_train.ravel())
        ...
        cprint(
            f"\n Done! {len(self._models)}/{len(self._method_names)} models trained.",
    ...
    def load(self, model_path: str) -> "AllML":
        ...
        cprint(f" Loaded: {model_path}", Colors.GREEN, bold=True)
    ...
    def pred(self, *args, method: Optional[str] = None):
        ...
        targets = ({method: self._models[method]} if method
                   else self._models)
        ...
        for mname, model in targets.items():
            try:
                ...
                results[mname] = raw.tolist() if hasattr(raw, "tolist") else raw
            except Exception as e:
        ...
        if len(results) == 1:
            ...
            for i, pv in enumerate(
                ...
                col = (self._pred_cols[i]
                       if i < len(self._pred_cols) else f"out_{i}")
...
{Colors.END}""")
        ...
        cprint(f"  Trained   : {self._is_trained}",
               Colors.GREEN if self._is_trained else Colors.RED)

        if self._split_done and self._X_train is not None:
            cprint(f"\n  Splits:", Colors.MAGENTA, bold=True)
            cprint(f"    Train: {self._X_train.shape[0]}", Colors.WHITE)
            cprint(f"    Test : {self._X_test.shape[0]}",  Colors.WHITE)
            if self._X_val is not None:
                cprint(f"    Val  : {self._X_val.shape[0]}", Colors.WHITE)
    ...
    def show_graph(self,
                   graph_type: Union[str, List[str]] = "all") -> "AllML":
    ...
    def __repr__(self):
        return (f"AllML(file='{self._filepath}', "
    ...
    def _detect_task_type(self):
        ...
        if n_tgt == 1:
            ...
            all_few = all(targets[c].nunique() <= 20 for c in self._pred_cols)
    ...
    def _build_model(self, mname, epochs, lr, n_estimators,
        ...
        """
        Build a model instance with safe hyperparameter mapping.

        Key fixes (v1.0.1):
        
         GradientBoosting    alpha skipped (its own (0,1) default kept)
         GradientBoosting    learning_rate as float 
         SGDRegressor/Cls    learning_rate="constant" + eta0=lr
         MLPRegressor/Cls    learning_rate="constant" + learning_rate_init=lr
         GaussianProcess     kernel left unset (no string kernel)
         IsotonicRegression  caught at fit time (needs 1-D)
        ...
        if task in (TaskType.REGRESSION,
                    TaskType.MULTI_OUTPUT_REGRESSION) and not is_reg:
            cprint(f"   ️  '{mname}' is not a regression method "
                   f"(task={task}).", Colors.YELLOW)
            cprint(f"       Try: {list(self.REGRESSION_METHODS.keys())[:4]}",
                   Colors.CYAN)
            return None
        ...
        if task in (TaskType.BINARY_CLASSIFICATION,
            ...
            cprint(f"   ️  '{mname}' is not a classification method "
                   f"(task={task}).", Colors.YELLOW)
            cprint(f"       Try: {list(self.CLASSIFICATION_METHODS.keys())[:4]}",
                   Colors.CYAN)
            return None
        ...
        #  learning_rate  (model-specific logic) 
        if mname in _FLOAT_LR:
            # These accept a plain float learning_rate
            if "learning_rate" in sig:
                init_params["learning_rate"] = float(lr)

        elif mname in _SGD_MODELS:
            # SGD wants a string schedule; actual step size goes to eta0
            if "learning_rate" in sig:
            ...
            # MLP wants a string schedule; initial rate goes to learning_rate_init
            if "learning_rate" in sig:
                init_params["learning_rate"] = "constant"
            ...
            # Generic fallback: set whichever param exists
            if "learning_rate" in sig:
                init_params["learning_rate"] = float(lr)
            if "learning_rate_init" in sig:
                init_params["learning_rate_init"] = float(lr)
        ...
        #  kernel 
        # Only SVM models accept a plain kernel string.
        # GaussianProcess expects a Kernel object  leave unset.
    ...
    def _evaluate(self, mname, model) -> Dict:
        ...
        try:
            ...
            # Isotonic needs 1-D input
            if mname == "isotonic_regression":
                if X_te.shape[1] != 1:
                    metrics["error"] = "IsotonicRegression needs 1 feature"
                    return metrics
    ...
    def _plot(self, graph_type, palette):
        if graph_type == "correlation_heatmap":
            ...
            cprint(f"️  Unknown graph: {graph_type}. Options: {self.GRAPH_OPTIONS}",
                   Colors.YELLOW)
    ...
    def _plot_correlation_heatmap(self):
        ...
        sns.heatmap(self._df[num_cols].corr(), annot=True, fmt=".2f",
    ...
    def _plot_distribution(self):
        ...
        for i, col in enumerate(cols):
            sns.histplot(self._df[col].dropna(), kde=True,
    ...
    def _plot_boxplot(self):
        ...
        for i, col in enumerate(num_cols):
            sns.boxplot(y=self._df[col].dropna(), ax=axes[i], color=pal[i])
    ...
    def _plot_confusion_matrix(self):
        ...
        if task not in (TaskType.BINARY_CLASSIFICATION,
                        TaskType.MULTICLASS_CLASSIFICATION):
            cprint("️  Confusion matrix: classification tasks only.",
                   Colors.YELLOW); return
        cms = {k: v for k, v in self._metrics.items() if "conf_matrix" in v}
        if not cms: return
        n = len(cms)
        fig, axes = plt.subplots(1, n, figsize=(7 * n, 6))
        if n == 1: axes = [axes]
        for ax, (mname, m) in zip(axes, cms.items()):
            sns.heatmap(m["conf_matrix"], annot=True, fmt="d",
    ...
    def _plot_roc_curve(self):
        if self._task_type != TaskType.BINARY_CLASSIFICATION:
            cprint("️  ROC Curve: binary classification only.", Colors.YELLOW)
            return
    ...
    def _plot_precision_recall(self):
        if self._task_type != TaskType.BINARY_CLASSIFICATION:
            cprint("️  PR Curve: binary classification only.", Colors.YELLOW)
            return
        fig, ax = plt.subplots(figsize=(8, 6))
        pal = sns.color_palette("husl", len(self._metrics))
        for i, (mname, m) in enumerate(self._metrics.items()):
        ...
        ax.set_title(" Precision-Recall", fontweight="bold"); ax.legend()
        plt.tight_layout(); plt.show()
    ...
    def _plot_feature_importance(self, palette):
        ...
        for mname, model in self._models.items():
            ...
            if hasattr(inner, "feature_importances_"):
                ...
                bars = ax.barh(self._feed_cols, imp,
                ...
                plt.tight_layout(); plt.show(); found = True

            elif hasattr(inner, "coef_"):
                coef = np.abs(inner.coef_.ravel()[:len(self._feed_cols)])
                fig, ax = plt.subplots(figsize=(10, 5))
                ax.barh(self._feed_cols[:len(coef)], coef,
                        color=sns.color_palette("husl", len(self._feed_cols)))
                ax.set_xlabel("|Coefficient|")
                ax.set_title(f" Coefficients: {mname}", fontweight="bold")
                plt.tight_layout(); plt.show(); found = True

        if not found:
            cprint("️  No models support feature importance.", Colors.YELLOW)

    def _plot_learning_curve(self):
        for mname, model in self._models.items():
            try:
                X_all = np.vstack([self._X_train, self._X_test])
                y_parts = [self._y_train, self._y_test]
                y_all = np.concatenate([
                    p.ravel() if (isinstance(p, np.ndarray)
                                  and p.ndim == 2 and p.shape[1] == 1)
                    else (p.ravel() if p.ndim == 1 else p)
                    for p in y_parts
                ])
                scoring = ("r2" if self._task_type in
                           (TaskType.REGRESSION,
                            TaskType.MULTI_OUTPUT_REGRESSION)
                           else "accuracy")
                train_sz, tr_sc, te_sc = learning_curve(
                    model, X_all, y_all, cv=3, n_jobs=-1,
                ...
                ax.plot(train_sz, tr_sc.mean(1), "o-",
                        color="blue",   label="Train")
                ax.plot(train_sz, te_sc.mean(1), "o-",
                        color="orange", label="CV")
                ax.set_xlabel("Samples"); ax.set_ylabel(scoring)
                ax.set_title(f" Learning Curve: {mname}", fontweight="bold")
                ax.legend(); plt.tight_layout(); plt.show()
            except Exception as e:
                cprint(f"️  Learning curve failed for {mname}: {e}",
                       Colors.YELLOW)

    def _plot_residuals(self, palette):
        if self._task_type not in (TaskType.REGRESSION,
                                   TaskType.MULTI_OUTPUT_REGRESSION):
            cprint("️  Residuals: regression only.", Colors.YELLOW); return
        n = len(self._metrics)
        ...
        plt.suptitle(" Residuals", fontsize=13, fontweight="bold")
        plt.tight_layout(); plt.show()

    def _plot_pred_vs_actual(self, palette):
        if self._task_type not in (TaskType.REGRESSION,
                                   TaskType.MULTI_OUTPUT_REGRESSION):
            cprint("️  Pred vs Actual: regression only.", Colors.YELLOW); return
        n = len(self._metrics)
    ...
    def _plot_error_distribution(self, palette):
        if self._task_type not in (TaskType.REGRESSION,
                                   TaskType.MULTI_OUTPUT_REGRESSION):
            cprint("️  Error dist: regression only.", Colors.YELLOW); return
        n = len(self._metrics)
        fig, axes = plt.subplots(1, n, figsize=(7 * n, 5))
        if n == 1: axes = [axes]
        for ax, (mname, m) in zip(axes, self._metrics.items()):
            if "y_pred" in m and "y_true" in m:
                err = np.array(m["y_true"]).ravel() - np.array(m["y_pred"]).ravel()
                sns.histplot(err, kde=True, ax=ax, color=palette[0])
                ax.axvline(0, color="red", linestyle="--")
                ax.set_title(f"Error Dist: {mname}", fontweight="bold")
    ...
    def _plot_cluster(self, palette):
        if self._task_type != TaskType.CLUSTERING:
            cprint("️  Cluster plot: clustering only.", Colors.YELLOW); return
        for mname, m in self._metrics.items():
            if "labels" not in m: continue
            fig, ax = plt.subplots(figsize=(9, 6))
            X2 = self._X_test[:, :2]
            sc = ax.scatter(X2[:, 0], X2[:, 1],
                            c=m["labels"], cmap="tab10", alpha=0.6)
            plt.colorbar(sc, ax=ax, label="Cluster")
            ax.set_title(f" Clusters: {mname}", fontweight="bold")
            plt.tight_layout(); plt.show()

    def _plot_model_comparison(self):
        if not self._metrics:
            cprint("️  No metrics to compare.", Colors.YELLOW); return
        task = self._task_type
        if task in (TaskType.REGRESSION, TaskType.MULTI_OUTPUT_REGRESSION):
            key, ylabel = "R2", "R² Score"
        elif task in (TaskType.BINARY_CLASSIFICATION,
                      TaskType.MULTICLASS_CLASSIFICATION,
                      TaskType.MULTI_OUTPUT_CLASSIFICATION):
            key, ylabel = "accuracy", "Accuracy"
        elif task == TaskType.CLUSTERING:
            key, ylabel = "silhouette", "Silhouette"
        else:
            return
        ...
        if not names:
            cprint(f"️  No '{key}' metric found.", Colors.YELLOW); return
